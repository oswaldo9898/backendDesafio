...
export default class Carts {
  ...
  addProductCart = async (cid, pid, cantidad) => {
    ...
    if (productIndex === -1) {
      const result = await cartModel.findByIdAndUpdate({ _id: cid }, { $push: { products: { "product": pid, "quantify": cantidad } } });
      return result;
    } else {
      const result = await cartModel.updateOne({ _id: cid, "products.product": pid }, { $inc: { "products.$.quantify": cantidad } });
      return result;
    }
  }
